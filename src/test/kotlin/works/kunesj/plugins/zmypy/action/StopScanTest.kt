...
import works.kunesj.plugins.zmypy.testutil.stopScan
...
class StopScanTest : AbstractToolWindowTestCase() {
    ...
    fun `test that we can stop an external process that runs an infinite loop`() {
        ...
        PlatformTestUtil.waitWhileBusy { !MypyScanJobRegistryService.getInstance(project).isActive() } // make sure that scan has been started
        stopScan(dataContext(project) {})
        PlatformTestUtil.waitWhileBusy { !MypyScanJobRegistryService.getInstance(project).isAvailable() }
    }
}