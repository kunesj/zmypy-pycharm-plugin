...
class IntentionTest : BasePlatformTestCase() {
    ...
    fun `test function annotated`() {
        ...
        val intention = myFixture.findSingleIntention(ZMypyBundle.message("mypy.intention.ignore.text", "valid-type"))
    ...
    fun `test single line triple-quoted string annotated with intention available`() {
        ...
        assertNotEmpty(myFixture.doHighlighting())
        val intention = myFixture.findSingleIntention(ZMypyBundle.message("mypy.intention.ignore.text", "name-defined"))
        assertNotNull(intention)
    }
